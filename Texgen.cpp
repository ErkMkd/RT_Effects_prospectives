...
using namespace std;
    ...
    GLuint Texgen::vbo_genCones=0;
    GLuint Texgen::vao_genCones=0;

    bool Texgen::init()
    {
        GLfloat sommets_texture[2*4]{-1.,-1., -1.,1., 1.,1., 1.,-1.};
        glGenBuffers(1,&vbo_genCones);
        glGenVertexArrays(1,&vao_genCones);
        glBindVertexArray(vao_genCones);
            glBindBuffer(GL_ARRAY_BUFFER,vbo_genCones);
                glBufferData(GL_ARRAY_BUFFER,sizeof(sommets_texture),sommets_texture,GL_STATIC_DRAW);
                glVertexAttribPointer(ShadersDemo::attrib_relief_genCones_position,2,GL_FLOAT,GL_FALSE,0,0);
                glEnableVertexAttribArray(ShadersDemo::attrib_relief_genCones_position);
        glBindVertexArray(0);

    }

    void Texgen::destruction()
    {
        if (glIsBuffer(vbo_genCones)) glDeleteBuffers(1,&vbo_genCones);
        if (glIsVertexArray(vao_genCones)) glDeleteVertexArrays(1,&vao_genCones);
    }
    ...
    void Texgen::init_dataSize()
    {
        ...
        switch (type)
        {
            ...
            case GL_UNSIGNED_INT: octets_par_composante=4;
        }

        switch (format)
        {
            case GL_LUMINANCE: octets_par_pixel=octets_par_composante*1;break;
    ...
    void Texgen::calcule_conesRelief_GPU(GLuint tex_profondeur_id,float echelle_profondeur,GLFWwindow* ecran)
    {
        ...
        glBindTexture(GL_TEXTURE_2D,tex_profondeur_id);

        glUseProgram(ShadersDemo::programme_relief_genCones);
        glUniform1i(ShadersDemo::uniform_relief_genCones_texture_profondeurs,0);
        glUniform2f(ShadersDemo::uniform_relief_genCones_taille_texture_sortie,float(largeur),float(hauteur));
        glUniform1f(ShadersDemo::uniform_relief_genCones_echelle_profondeur,echelle_profondeur);
