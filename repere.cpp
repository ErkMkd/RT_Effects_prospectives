#include "repere.h"
	...
	void Repere::initEchelle(float p_ex, float p_ey, float p_ez)
	{
	    ex=p_ex;ey=p_ey;ez=p_ez;
	}
	void Repere::initEchelle(float e[3])
	{
    ...
    {
        glm::mat4 trans=glm::translate(glm::mat4(1.),glm::vec3(x,y,z));
		glm::mat4 rot_l=glm::rotate(trans,l*float(DEG_RAD),glm::vec3(0.,1.,0.));
		glm::mat4 rot_t=glm::rotate(rot_l,t*float(DEG_RAD),glm::vec3(1.,0.,0.));
        glm::mat4 rot_r=glm::rotate(rot_t,r*float(DEG_RAD),glm::vec3(0.,0.,1.));
		matrice=glm::scale(rot_r,glm::vec3(ex,ey,ez));
    }

    void Repere::calcul_matrice_normale()
    {
		matrice_normale=glm::inverseTranspose(glm::mat3(matrice));
    }
